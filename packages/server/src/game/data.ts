...
// prize (highest passing total wins). successMessage/failureMessage are shown on the
// results screen next to that approach's outcome — one told per family per attended
// scenario, based on whether their check passed.
export const DEFAULT_SCENARIOS: ScenarioDesign[] = [
  // General
  {
    emoji: '🐎', title: 'Bandit Raid', description: 'Bandits are terrorising the roads around {town}, and the merchants have put a bounty on their stolen loot.', location: 'general', reward: { type: 'gold', amount: 20 }, approaches: [
      { label: 'Ride them down', skill: 'might', successMessage: 'Steel flashes and the bandits break, leaving their plunder behind in the mud.', failureMessage: 'The bandits circle back before the line can hold, and the loot goes with them.' },
      { label: 'Infiltrate their camp', skill: 'cunning', successMessage: 'A false trail is laid, and the bandits are led away from their own hoard.', failureMessage: 'The disguise slips at the worst moment, and the camp turns hostile.' },
    ],
  },
  {
  ...
  },
  {
    emoji: '💎', title: 'Missing Heirloom', description: 'A precious relic has vanished in {town}. The reward for its return is generous.', location: 'general', reward: { type: 'gold', amount: 30 }, approaches: [
      ...
      { label: 'Simply buy it back, no questions asked', buyoutCost: 30, successMessage: 'A fat purse loosens tongues, and the relic changes hands quietly before sundown.', failureMessage: 'Word of the offer spreads faster than the relic does, and someone else buys it first.' },
    ],
  },
  {
    emoji: '⚖️', title: 'Trade Dispute', description: 'Merchants of {town} are at each other’s throats over a fortune in cargo.', location: 'general', reward: { type: 'gold', amount: 20 }, approaches: [
      ...
      { label: 'Simply buy out both merchants’ claims', buyoutCost: 20, successMessage: 'Coin settles what argument could not, and the cargo is yours to sell on.', failureMessage: 'A third merchant swoops in with a better offer before the deal is signed.' },
    ],
  },
  {
    emoji: '🕊️', title: 'Peace Talks', description: 'Feuding lords meet at {town} under a banner of truce.', location: 'general', approaches: [
      ...
      { label: 'Simply buy both lords’ compliance', buyoutCost: 30, successMessage: 'Gold changes hands under the table, and both lords discover peace suits them after all.', failureMessage: 'One lord pockets the gold and walks anyway, insulted the other was paid more.' },
    ],
  },
  {
  ...
  },
  {
    emoji: '🕯️', title: 'Whispers of Treason', description: 'A plot brews in the back rooms of {town}.', location: 'general', approaches: [
      ...
      { label: 'Simply pay the plotters to turn on each other', buyoutCost: 20, successMessage: 'Gold proves a better argument than loyalty, and the plotters betray one another by morning.', failureMessage: 'The plotters take the coin, promise everything, and deliver nothing.' },
    ],
  },
  {
    emoji: '💰', title: "Smugglers' Cache", description: 'Rumour says a smugglers’ cache lies hidden somewhere near {town}, still unclaimed.', location: 'general', reward: { type: 'gold', amount: 30 }, approaches: [
      { label: "Track the smugglers' route", skill: 'wit', successMessage: 'The trail of hoofprints and broken twigs leads straight to the cache.', failureMessage: 'The trail goes cold at a stream crossing, and the cache stays hidden.' },
      { label: 'Muscle the truth out of a lookout', skill: 'might', successMessage: 'A firm hand and a firmer glare loosen the lookout’s tongue soon enough.', failureMessage: 'The lookout would rather take a beating than talk, and says nothing.' },
      { label: 'Simply buy the map off a turncoat smuggler', buyoutCost: 20, successMessage: 'The map is genuine, and the cache is exactly where it promises.', failureMessage: 'The "map" turns out to be an old bar tab sketched on the back — a costly joke.' },
    ],
  },
  // Capital only
  {
  ...
  },
  {
    emoji: '🏰', title: 'Royal Audience', description: 'The crown grants audiences at {town}. Favour hangs in the balance.', location: 'capital', approaches: [
      ...
      { label: 'Call in a favour from an old ally at court', skill: 'cunning', successMessage: 'A quiet word from a well-placed friend, and the audience is granted at once.', failureMessage: 'The old ally has fallen out of favour too, and can do nothing after all.' },
      { label: 'Simply bribe the chamberlain outright', buyoutCost: 20, successMessage: 'A quiet exchange in the antechamber, and the audience goes exactly as planned.', failureMessage: 'The chamberlain pockets the bribe and does nothing whatsoever in return.' },
    ],
  },
  {
  ...
  },
  {
    emoji: '📜', title: 'Ledgers & Accounts', description: 'The estate books at {town} are in disarray — and the tax collector is due.', location: 'home', reward: { type: 'gold', amount: 20 }, approaches: [
      ...
      { label: 'Simply pay the shortfall out of pocket', buyoutCost: 20, successMessage: 'The collector counts the coin, nods once, and troubles the estate no further.', failureMessage: 'The collector counts it twice, finds it short anyway, and levies a fine besides.' },
    ],
  },
  {
  ...
  {
    emoji: '🗝️', title: 'Buried Family Fortune', description: "Family legend swears a strongbox lies buried somewhere on the {town} estate.", location: 'home', reward: { type: 'gold', amount: 30 }, approaches: [
      { label: "Puzzle out grandfather's old riddle", skill: 'wit', successMessage: 'The riddle finally clicks, and the spade strikes wood on the very first try.', failureMessage: 'The riddle turns out to describe a landmark torn down decades ago.' },
      { label: 'Dig up every likely spot yourself', skill: 'might', successMessage: 'A long, filthy afternoon of digging finally turns up the strongbox.', failureMessage: 'The estate is left pockmarked with holes, and not one of them holds anything.' },
      { label: 'Hire a diviner to point the way', buyoutCost: 20, successMessage: 'The diviner’s rod dips sharply, and the strongbox is exactly where it points.', failureMessage: 'The diviner is confident, theatrical, and completely wrong.' },
    ],
  },
]
